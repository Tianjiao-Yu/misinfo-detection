...
def precision(y_true, y_pred):
    ...
    precision = true_positives / (predicted_positives + K.epsilon()) ##use epsilon avoid dividing zero
    return precision
...
def dot_product(x, kernel):
    ## be compatible with both Theano and Tensorflow
...
def preprocess_data(data_frame):
    ...
    data_frame = data_frame[['content', 'credibility','title']]
    shuffle(data_frame).reset_index()
    data_frame =data_frame[~data_frame['content'].isnull()]
    log("number of labels: "+str(len(data_frame['credibility'].unique())))
    return data_frame
